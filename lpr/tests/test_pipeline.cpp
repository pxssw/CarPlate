...
#include <iostream>  
#include "io.h"
void filesearch1(string path, int layer, vector <string> &filepathnames)
{
	...
	while (!(done = _findnext(handle, &filefind)))
	{
		...
		if ((_A_SUBDIR == filefind.attrib))
		{
			...
			filesearch1(curr, layer + 1, filepathnames);
		}
		else
		{
			curr = path + "\\" + filefind.name;
...
void TEST_PIPELINE() {
	...
	vector<string> files;
	filesearch1(picture_path, 0, files);

	for (int i = 0; i < files.size(); i++)
	{
		string input_img_dir = files[i];
		cv::Mat image = imread(input_img_dir);
